...
public class HRTool extends AbstractBaseTool<HRTool.HrToolInput> {
    ...
    static {
        ...
        // 初始化入职指南
        ONBOARDING_GUIDES.put("default", Arrays.asList(
            "1. 签署劳动合同",
            "2. 办理入职手续",
            "3. 领取办公设备",
            "4. 加入公司通讯群",
        ...
        ONBOARDING_GUIDES.put("研发部", Arrays.asList(
            "1. 签署劳动合同与保密协议",
            "2. 领取 MacBook / 高配PC",
            "3. 配置 Git / Jenkins / 内部系统权限",
            "4. 加入研发钉钉群 & 代码库",
            "5. 完成安全与编码规范培训"
        ));
        ONBOARDING_GUIDES.put("产品部", Arrays.asList(
            "1. 签署劳动合同",
            "2. 领取办公设备",
            "3. 配置 Jira / Confluence / 数据看板权限",
            "4. 参加产品方法论培训",
            "5. 对接业务方负责人"
        ));
    }
    ...
    private static final Set<String> SUPPORTED_ACTIONS = Set.of(
        ...
        "check_offboarding_status",
        "create_recruitment_request",
        "query_recruitment_requests",
        "query_recruitment_progress",
        ...
        if (!SUPPORTED_ACTIONS.contains(action)) {
            return new ToolExecuteResult(
                "❌ Unsupported action: '" + action + "'. " +
                    "Supported actions: " + String.join(", ", SUPPORTED_ACTIONS)
            );
        }
    ...
    private ToolExecuteResult handleQueryEmployee(HrToolInput input) {
        ...
        if (id != null) {
            ...
            if (emp != null) matches.add(emp);
        } else if (name != null) {
            matches = MOCK_EMPLOYEES.values().stream()
        ...
        for (Employee e : matches) {
            sb.append("- ID: ").append(e.id)
                ...
                .append(", Dept: ").append(e.department)
                .append(", Position: ").append(e.position)
                .append(", Email: ").append(e.email)
                .append("\n");
        }
    ...
    private ToolExecuteResult handleAddEmployee(HrToolInput input) {
        ...
        log.info("📝 Simulated add_employee: name={}, dept={}, position={}, start_date={}", name, dept, position, startDate);
        return new ToolExecuteResult(
            "✅ Simulated employee addition draft created:\n" +
                "- Name: " + name + "\n" +
                "- Department: " + dept + "\n" +
                "- Position: " + position + "\n" +
    ...
    private ToolExecuteResult handleRemoveEmployee(HrToolInput input) {
        ...
        return new ToolExecuteResult(
            "⚠️ Simulated offboarding initiated for:\n" +
                "- Name: " + emp.name + "\n" +
                "- ID: " + id + "\n" +
    ...
    private ToolExecuteResult handleGetOnboardingGuide(HrToolInput input) {
        ...
        if (dept != null) sb.append(" for ").append(dept);
        sb.append(":\n");
    ...
    private ToolExecuteResult handleCheckOnboardingStatus(HrToolInput input) {
        ...
        // 模拟状态
        return new ToolExecuteResult(
            "✅ Onboarding status for " + emp.name + " (" + id + "):\n" +
                "- Contract Signed: ✅\n" +
    ...
    private ToolExecuteResult handleCheckOffboardingStatus(HrToolInput input) {
        ...
        return new ToolExecuteResult(
            "✅ Offboarding status for " + emp.name + " (" + id + "):\n" +
                "- Exit Interview: ✅\n" +
    ...
    private ToolExecuteResult handleCreateRecruitmentRequest(HrToolInput input) {
        ...
        if (dept == null || dept.isEmpty()) return new ToolExecuteResult("❌ 'department' is required.");
        if (position == null || position.isEmpty()) return new ToolExecuteResult("❌ 'position' is required.");
        if (headcount == null || headcount <= 0) return new ToolExecuteResult("❌ 'headcount' must be a positive integer.");
        if (reason == null || reason.isEmpty()) return new ToolExecuteResult("❌ 'reason' is required.");

        String reqId = "REQ-" + (MOCK_RECRUITMENT_REQUESTS.size() + 100);
        log.info("📝 Simulated recruitment request: dept={}, pos={}, hc={}, reason={}", dept, position, headcount, reason);
        return new ToolExecuteResult(
            "✅ Simulated recruitment request draft created:\n" +
                "- Request ID: " + reqId + "\n" +
                "- Department: " + dept + "\n" +
                "- Position: " + position + "\n" +
                "- Headcount: " + headcount + "\n" +
    ...
    private ToolExecuteResult handleQueryRecruitmentRequests(HrToolInput input) {
        ...
        for (RecruitmentRequest r : filtered) {
            sb.append("- ID: ").append(r.id)
                .append(", Dept: ").append(r.department)
                .append(", Position: ").append(r.position)
                .append(", HC: ").append(r.headcount)
    ...
    private ToolExecuteResult handleQueryRecruitmentProgress(HrToolInput input) {
        ...
        // 模拟进度
        return new ToolExecuteResult(
            "✅ Recruitment progress for '" + position + "'" + (dept != null ? " in " + dept : "") + ":\n" +
                "- Total Requests: 1\n" +
                "- Resumes Received: 42\n" +
                "- Phone Screens: 15\n" +
                "- Tech Interviews: 8\n" +
    ...
    private ToolExecuteResult handleQueryTeamMembers(HrToolInput input) {
        ...
        List<Employee> members = MOCK_EMPLOYEES.values().stream()
    ...
    public String getCurrentToolStateString() {
        return "HrTool State:\n"
            ...
            + "- Mock Employees: " + MOCK_EMPLOYEES.size() + "\n"
