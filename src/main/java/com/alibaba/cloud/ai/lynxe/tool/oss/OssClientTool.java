...
public class OssClientTool extends AbstractBaseTool<OssClientTool.OssClientInput> {
    ...
    static {
        ...
        OSS_CONFIGS.put("qiniu", new OssConfig(
            "s3.cn-east-1.qiniucs.com", // Endpoint not strictly required for Qiniu SDK auto-region
            "IAM-d-sLwdqfZ0KI0z9F5IKsRfXTlY-cSwn_qupifXch",
            "uFnwkexXnlwF_tWbm4u9wRrynBfsJbip09FEGmTqRPxz",
            "exe-test",
            "https://t-company-res.exexm.com"
        ));
    }
    ...
    public ToolExecuteResult run(OssClientInput input) {
      AuthContext.setContextPlanId(rootPlanId);
       ...
       String fileName = input.fileName;
        if (StringUtils.isNullOrEmpty(fileName) && ("upload".equalsIgnoreCase(input.operation) || "replace".equalsIgnoreCase(input.operation))) {
    ...
    private ToolExecuteResult handleUpload(OssClientInput input, OssConfig config, String ossType) throws Exception {
      ...
      if (sourceFile == null || !Files.exists(sourceFile)) {
        ...
        if ("aliyun".equals(ossType)) {
            ...
            try {
                PutObjectRequest putObjectRequest = new PutObjectRequest(config.bucketName, objectKey, Files.newInputStream(sourceFile));
            ...
            while (fileListIterator.hasNext()) {
                ...
                for (com.qiniu.storage.model.FileInfo item : items) {
                    result.append("- [").append(item.key).append("]")
                      .append("(").append(config.domain).append("/").append(item.key).append("\n");
                }
            }
  ...
  private String normalizeFilePath(String filePath) {
    ...
    private static class OssConfig {
        ...
        public OssConfig(String endpoint, String accessKeyId, String accessKeySecret, String bucketName,String domain) {
